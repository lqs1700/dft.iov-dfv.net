...
$start = "select count(*) from app_dataman_receipt";
$end = " order by receipttime desc";
if($devid){
	$start.=" WHERE devid = '$devid'";
	if($start_time){
		$start.=" AND substr(receipttime,1,10)>= '$start_time'";
	}
	if($end_time){
		$start.=" AND substr(receipttime,1,10)<= '$end_time'";
	}
}
if(!$devid){
	if($start_time){
		$start.=" WHERE substr(receipttime,1,10)>= '$start_time'";
		if($end_time){
			$start.=" AND substr(receipttime,1,10)<= '$end_time'";
		}
	}elseif($end_time){
		$start.=" WHERE substr(receipttime,1,10)<= '$end_time'";
	}
}
$sql = $start.$end;
$res = DB($sql);
...
$start = "select * from app_dataman_receipt";
$end = " order by receipttime desc limit $offet,$page_size";
if($devid){
	$start.=" WHERE devid = '$devid'";
	if($start_time){
		$start.=" AND substr(receipttime,1,10)>= '$start_time'";
...
$key.='</div>'; 
require "../html/tuisongliebiao.html";