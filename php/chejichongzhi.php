...
if($page!=1){
    $key.="<a href=\"".$_SERVER['PHP_SELF']."?time=$time&&page=1\">第一页</a> ";
    $key.="<a href=\"".$_SERVER['PHP_SELF']."?time=$time&&page=".($page-1)."\">上一页</a>";
}else {
    $key.="第一页 ";
    $key.="上一页";
}
...
if($page!=$pages){
    $key.=" <a href=\"".$_SERVER['PHP_SELF']."?time=$time&&page=".($page+1)."\">下一页</a> ";
    $key.="<a href=\"".$_SERVER['PHP_SELF']."?time=$time&&page={$pages}\">最后一页</a>";
}else {
    $key.="下一页 ";
    $key.="最后一页";
}
$key.='</div>';
require_once "../html/chejichongzhi.html";